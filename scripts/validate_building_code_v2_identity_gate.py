#!/usr/bin/env python3
"""Validate V2 Gate 1 building identity test cases and sample packets."""
...
SCHEMA_PATH = V2_DATA_DIR / "building-identity-schema.json"
TEST_CASES_PATH = V2_DATA_DIR / "test-cases.json"
...
FORBIDDEN_IDENTITY_CONCLUSION_PATTERNS = {
    "earthquake safety conclusion": [
        re.compile(r"\b(earthquake|seismic(?:ally)?)\s+(safe|unsafe|safety)\b", re.IGNORECASE),
        re.compile(r"\b(safe|unsafe)\s+(from|against|for)\s+earthquakes?\b", re.IGNORECASE),
        re.compile(r"\b(is|are|was|were|looks|seems|declared|deemed)\s+(safe|unsafe)\b", re.IGNORECASE),
        re.compile(r"\bnot\s+safe\b", re.IGNORECASE),
    ],
    "NSCP or seismic design conclusion": [
        re.compile(
            r"\b(NSCP|seismic design)\s+(compliant|compliance|approved|verified|confirmed|proven|proved)\b",
            re.IGNORECASE,
        ),
        re.compile(r"\b(complies?|compliant)\s+(with\s+)?NSCP\b", re.IGNORECASE),
    ],
    "OBO structural review conclusion": [
        re.compile(
            r"\bOBO\b.*\b(structural\s+)?(approved|cleared|reviewed|verified|confirmed|proven|proved)\b",
            re.IGNORECASE,
        ),
        re.compile(r"\b(structural\s+)?review\s+(approved|passed|cleared|confirmed)\b", re.IGNORECASE),
    ],
    "post-earthquake tag status conclusion": [
        re.compile(r"\b(red|yellow|green)\s*-?\s*tag(?:ged)?\b", re.IGNORECASE),
        re.compile(r"\b(post[- ]earthquake\s+)?tag\s+status\s+(is|confirmed|cleared)\b", re.IGNORECASE),
    ],
    "clearance conclusion": [
        re.compile(r"\b(officially\s+)?cleared\s+(after|for|by|from)\b", re.IGNORECASE),
        re.compile(r"\bclearance\s+(approved|issued|confirmed|verified|found|exists)\b", re.IGNORECASE),
    ],
    "compliance or noncompliance conclusion": [
        re.compile(r"\b(is|are|was|were|declared|deemed)\s+(compliant|noncompliant|non-compliant)\b", re.IGNORECASE),
        re.compile(r"\bnot\s+compliant\b", re.IGNORECASE),
        re.compile(r"\bcompliance\s+(confirmed|proven|proved|verified)\b", re.IGNORECASE),
    ],
    "permit existence conclusion": [
        re.compile(
            r"\b(building\s+|occupancy\s+)?permit\s+"
            r"(exists|does\s+not\s+exist|found|confirmed|verified|proven|proved|approved|issued|missing)\b",
            re.IGNORECASE,
        ),
        re.compile(r"\b(no|without)\s+(building\s+|occupancy\s+)?permit\b", re.IGNORECASE),
        re.compile(r"\bhas\s+(a\s+)?(building\s+|occupancy\s+)?permit\b", re.IGNORECASE),
    ],
    "ownership proof conclusion": [
        re.compile(r"\b(ownership|owner)\s+(proved|proven|confirmed|verified)\b", re.IGNORECASE),
        re.compile(r"\bproves?\s+ownership\b", re.IGNORECASE),
    ],
    "construction quality conclusion": [
        re.compile(
            r"\b(construction quality|quality of construction)\s+"
            r"(proved|proven|confirmed|verified|good|poor|acceptable|unacceptable)\b",
            re.IGNORECASE,
        ),
        re.compile(r"\b(well|poorly)[- ]built\b", re.IGNORECASE),
    ],
}


def fail(message: str) -> int:
    print(f"building code V2 identity gate validation failed: {message}", file=sys.stderr)
    return 1
...
def validate_no_forbidden_identity_conclusions(value: object, path: str, errors: list[str]) -> None:
    for string_path, text in walk_strings(value, path):
        for conclusion_type, patterns in FORBIDDEN_IDENTITY_CONCLUSION_PATTERNS.items():
            for pattern in patterns:
                if pattern.search(text):
                    errors.append(
                        f"{string_path}: identity evidence must not assert {conclusion_type}: {text!r}"
                    )
                    break


def validate_candidate_generated_conclusions(value: object, path: str, errors: list[str]) -> None:
    ...
    for field in generated_fields:
        if field in obj:
            validate_no_forbidden_identity_conclusions(obj.get(field), f"{path}.{field}", errors)
    ...
    if isinstance(evidence_items, list):
        for index, evidence_item in enumerate(evidence_items):
            ...
            if not isinstance(evidence_item, dict):
                continue
            validate_no_forbidden_identity_conclusions(evidence_item.get("claim"), f"{evidence_path}.claim", errors)
            validate_no_forbidden_identity_conclusions(evidence_item.get("notes"), f"{evidence_path}.notes", errors)
            source = evidence_item.get("source")
            if isinstance(source, dict):
                validate_no_forbidden_identity_conclusions(source.get("label"), f"{evidence_path}.source.label", errors)


def validate_packet_generated_conclusions(packet: dict, packet_path_value: str, errors: list[str]) -> None:
    candidates = packet.get("identity_candidates")
    if isinstance(candidates, list):
        for index, candidate in enumerate(candidates):
            validate_candidate_generated_conclusions(
                candidate,
                f"sample[{packet_path_value}].identity_candidates[{index}]",
                errors,
            )
    ...
    if isinstance(selected_candidate, dict):
        validate_candidate_generated_conclusions(
            selected_candidate,
            f"sample[{packet_path_value}].selected_candidate",
            errors,
        )
    ...
    if isinstance(confirmation, dict):
        ...
        for field in output_fields:
            validate_no_forbidden_identity_conclusions(
                confirmation.get(field),
                f"sample[{packet_path_value}].confirmation.{field}",
                errors,
            )
...
def validate_case(case: dict) -> list[str]:
    ...
    try:
        packet_path.relative_to(V2_DATA_DIR.resolve())
    except ValueError:
        return [
            f"{case_id}: sample_packet_path must stay under "
            f"{V2_DATA_DIR.relative_to(ROOT)!s}, got {packet_path_value!r}"
        ]
    ...
    validate_packet_generated_conclusions(packet, packet_path_value, errors)
