...
The internal adapter layer may use `scripts/tracker_session_start.py` as the session wrapper and `scripts/tracker_workflow_start.py`, `scripts/tracker_workflow_checkpoint.py`, `scripts/tracker_workflow_close.py`, and `scripts/tracker_project_situation.py` as workflow-run primitives. These are implementation details behind the skill surface, not operator instructions.
